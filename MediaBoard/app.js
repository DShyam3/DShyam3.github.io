...
function setupEventListeners() {
    ...
    if (refreshBtn) {
        refreshBtn.addEventListener('click', async () => {
            ...
            try {
                const changes = await updateService.checkForUpdates(true);
...
function updateFilterOptions(filterId, options, items, field) {
    ...
    options.forEach(option => {
        const count = items.filter(item => item[field].includes(option)).length;
        filter.appendChild(new Option(`${option} (${count})`, option));
    });
...
function filterMovies(movies) {
    ...
    // Then sort based on release status
    if (currentFilters.movies.releaseStatus === 'coming_soon') {
        // Sort upcoming movies by release date (ascending)
        ...
        // Keep alphabetical sorting for other views
        return filteredMovies.sort((a, b) => 
            a.title.toLowerCase().localeCompare(b.title.toLowerCase())
        );
    }
